...
window.onload = function init( ) {
	...
	// Set uGouraud, uPhong, checkGourad and checkPhong initially
	var checkGouraudLoc = gl.getUniformLocation( program, "checkGouraud" );
	gl.uniform1iv( checkGouraudLoc, [ 1 ] );
	var checkPhongLoc = gl.getUniformLocation( program, "checkPhong" );
	gl.uniform1iv( checkPhongLoc, [ 0 ] );
...
function render(now) {
	...
	//Set the transformations, Rotations and Center of Rotation to be used for the Merry Go Round
		
	Rotation = rotateY(theta);
	gl.uniformMatrix4fv(vRotate, false, flatten(Rotation));
	
	centerOfRotation=vec3(-0.5,0,0);
	gl.uniform3fv(vCenter, centerOfRotation);
	...
	var lightPosition;
	// Draw a sphere
	transformation = mult( translate(-0.5, 1.7, 0), scalem( 0.2, 0.2, 0.2 ) );
	if(sphereCheck){
	...
	// Draw mid Cones
	transformation = mult( translate(-0.5, 0.7, 0), scalem( 0.2, 0.6, 0.2 ) );
	if(coneCheck){
	...
	transformation = mult( translate(-0.5, 0.7, 0), mult(rotateX(180), scalem( 0.2, 0.6, 0.2 )) );
	gl.uniformMatrix4fv(vTransformation, false, flatten(transformation));
	renderCone( midConeBuffers, nConeSectors, gl, program);
	...
	// Top Of Cone
	transformation = mult( translate(-0.5, 1.2, 0), scalem( 1, 0.5, 1 ) );
	...
	//Bottom Of Cone
	transformation = mult( translate(-0.5, 0, 0), scalem( 1, -0.1, 1 ) );
	// Set the Normal Matrix for our object
	gl.uniformMatrix3fv( uNormalMatrixLoc, false, flatten( calcNormalMatrix( modelView, transformation ) ) );
	gl.uniformMatrix4fv(vTransformation, false, flatten(transformation));
	renderCone( coneBuffers, nConeSectors2, gl, program);
	...
	horseTranslations.push(translate(0.25, 0, 0));
	horseTranslations.push(translate(-0.5, 0, 0.75));
	horseTranslations.push(translate(-1.25, 0, 0));
	horseTranslations.push(translate(-0.5, 0, -0.75));
